...
void parseArgs( int argc, char** argv, Args& arg ) {
    ...
    if(arg.outfile == "") arg.outfile = arg.reference + "_" + arg.collection;
...
std::pair<int,int> contractLeft(int lo, int hi, int offset){
    ...
    if(suflo == _sizeReference-1 || sufhi == _sizeReference-1){ //if true we must be at depth 1
...
inline int32_t binarySearchLB(int32_t lo, int32_t hi, uint32_t offset, uint8_t c) {
...
inline int32_t binarySearchRB(int32_t lo, int32_t hi, uint32_t offset, uint8_t c) {
   int32_t low = lo, high = hi;
   while (low <= high) {
      int32_t mid = (low + high) >> 1;
      uint8_t midVal = reference[_SA[mid] + offset];
...
void computeMatchingFactor(const std::string &collection, uint64_t i, int32_t *pos, int32_t *len, int32_t & leftB, int32_t & rightB){
    uint32_t offset = *len;
    uint64_t j = i + offset;

    int32_t nlb = leftB, nrb = rightB, maxMatch;
    unsigned int match = _SA[nlb];
    while (j < _sn) { //scans the string from j onwards until a maximal prefix is found between reference and collection
        if (nlb == nrb) {
...
void loadReferenceAndComputeDS(std::string refFileName){
    ...
    if(n){
        ...
        if(firstChar[0] == '>'){
            ...
            if(content.size()) reference += content;
            std::string().swap(content);
    ...
    libsais(reinterpret_cast<unsigned char*>(const_cast<char*>(reference.c_str())), sa, reference.size(), 0, NULL);
    auto t02 = std::chrono::high_resolution_clock::now();
    printf("Computing SA done in %ld ms\n", std::chrono::duration_cast<std::chrono::milliseconds>(t02 - t01).count());
    ...
    printf("Computing ISA done in %ld ms\n", std::chrono::duration_cast<std::chrono::milliseconds>(t02 - t01).count());

    libsais_plcp(reinterpret_cast<unsigned char*>(const_cast<char*>(reference.c_str())), _SA, _LRF, reference.size());
    ...
    t02 = std::chrono::high_resolution_clock::now();
    printf("Computing LCP and PLCP done in %ld ms\n", std::chrono::duration_cast<std::chrono::milliseconds>(t02 - t01).count());

    t01 = std::chrono::high_resolution_clock::now();
    fprintf(stderr,"\tComputing RMQ...\n");
    _rmq = new rmq_tree((int *)_LCP, reference.size(), 7);
    t02 = std::chrono::high_resolution_clock::now();
    printf("Computing RMQ done in %ld ms\n", std::chrono::duration_cast<std::chrono::milliseconds>(t02 - t01).count());
...
void loadReferenceAndComputeDS_parallel(std::string refFileName, uint32_t numThreads = 1){
    ...
    auto t01 = std::chrono::high_resolution_clock::now();
    int32_t *sa = new int32_t[reference.size()];
    libsais_omp(reinterpret_cast<unsigned char*>(const_cast<char*>(reference.c_str())), sa, reference.size(), 0, NULL, numThreads);
    auto t02 = std::chrono::high_resolution_clock::now();
    printf("Computing SA done in %ld ms\n", std::chrono::duration_cast<std::chrono::milliseconds>(t02 - t01).count());
    ...
    libsais_plcp_omp(reinterpret_cast<unsigned char*>(const_cast<char*>(reference.c_str())), _SA, _LRF, reference.size(), numThreads);
...
std::vector<std::pair<uint32_t, uint32_t>> computeMatchingStatistics(std::string collFileName, uint64_t bufferText, std::string outputFileName = ""){
    ...
    while(std::getline(streamInfile, line).good()){
        if( line.empty() || line[0] == '>' ){
            ...
            D++;
            while(i < (static_cast<int64_t>(content.size()))-1){ //stop before the separator
                ...
                if(matchingStatistics.size() >= capBufferWrite){
                    streamOutfile.write(reinterpret_cast<char*>(matchingStatistics.data()), matchingStatistics.size()*sizeof(std::pair<uint32_t, uint32_t>));
                ...
                if(leftB == rightB){
                    ...
                    std::pair<int,int> interval = adjustInterval(_ISA[pos+1], _ISA[pos+1], len);
...
std::vector<std::pair<uint32_t, uint32_t>> computeMatchingStatisticsParallel(std::string collFileName, uint64_t bufferText, std::string outputFileName = "", uint32_t numThreads = 1){
    ...
    while(charactersRead < _sn){
        ...
        for(uint32_t i = 0; i < numThreads; i++){
            streamOutfiles[i].open(outputFileName + std::to_string(i), std::ios::out | std::ios::binary);
            if(!streamOutfiles[i]){
        ...
        for(uint32_t thread = 0; thread < numThreads; thread++){
            ...
            while(i < parts[thread+1]){ 
                if(content[i] == sequenceSeparator){
                    ...
                    leftB = 0;
                    rightB = _sizeReference-1;
                    matchingStatistics[omp_get_thread_num()].push_back(std::make_pair(pos, len));
                    i++;
                    continue;
                }
                computeMatchingFactor(content, i, &pos, &len, leftB, rightB);
                matchingStatistics[omp_get_thread_num()].push_back(std::make_pair(pos, len));
                if(matchingStatistics[omp_get_thread_num()].size() >= capBufferWrite/numThreads){
                    ...
                    matchingStatistics[omp_get_thread_num()].reserve(capBufferWrite/numThreads);
                }
                len--;
                ...
                if(leftB == rightB){
                    while((len > _LRF[pos+1]) & (i < parts[thread+1])){
                        time++;
                        matchingStatistics[omp_get_thread_num()].push_back(std::make_pair(pos+1, len));
                        i++;
                        len--;
                        pos++;
                    }
        ...
        // unite the files
        for(uint32_t thread = 0; thread < numThreads; thread++){
            std::ifstream streamInfileThread(outputFileName + std::to_string(thread), std::ios::in | std::ios::binary);
            if(!streamInfileThread){
                fprintf(stderr, "Error opening file of sequence (%s)\n", (outputFileName + std::to_string(thread)).c_str());
                exit(1);
            }
            // std::cout << "file " << outputFileName + std::to_string(thread) << " size: " << std::filesystem::file_size(outputFileName + std::to_string(thread)) << '\n';
            if(std::filesystem::file_size(outputFileName + std::to_string(thread)) == 0){
                std::cout << "file " << outputFileName + std::to_string(thread) << " is empty\n";
